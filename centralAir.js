...
export function keyMain(key) {
    sendSignals(getRemote(KeyToBinary(key)));
}

function KeyToBinary(state) {
    var code = '' //decoding in binary bit

    //the first half signals
...
function sendSignals(remote) {
    fs.writeFile('./AIR.lircd.conf', remote, err => {
        ...
        //     console.log(error)
        // })
        // cmd.run('sudo systemctl stop lircd');

        // setTimeout(() => {
        // console.log("Command sent");
        // exec('irsend SEND_ONCE AIR command'), (err, stdout, stderr) => {
        //     console.log(err)
        // }
        // }, 500);
    })
